...
export default function Login() {
  ...
  const sendVerification = async () => {
    ...
    try {
      ...
      toast({ title: "Verification sent", description: "A verification code was generated. Check your email (or dev server response).", variant: "default" });
    } catch (err: any) {
  ...
  const submit = async (e: any) => {
    ...
    try {
      ...
      const msg = err?.error || err?.message || JSON.stringify(err) || "Action failed";
      toast({ title: mode === "login" ? "Login error" : "Signup error", description: String(msg), variant: "destructive" });
    }
  };
      ...
      <div className="mx-auto max-w-md">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="text-sm text-muted-foreground">
            <button className="underline" onClick={() => setMode(mode === "login" ? "register" : "login")}>{mode === "login" ? "Create an account" : "Have an account?"}</button>
          </div>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4 rounded-lg border bg-card p-6">
          ...
          <div>
            ...
            <input type="email" className="mt-1 w-full rounded-md border px-3 py-2" value={email} onChange={(e) => { setEmail(e.target.value); setEmailVerified(false); setVerificationSent(false); }} required />
          </div>
